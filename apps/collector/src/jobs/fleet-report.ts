 ...
 * snapshot's own ruleset version, with at least one evidence link, and every
 * evidence link must reference an observation from this snapshot. Anything
 * else must not drive a PASS.
 */
// Export for testing
export function isCorrelatedFinding(
  ...
  return (
    ...
    finding.evidence.every((link) => snapshotObservationIds.has(link.observationId))
  );
}
...
export function generateSummary(
  ...
  const summary: Record<string, unknown> = {
    ...
    // Unknown checks are incomplete evidence, not actionable issues, but they
    // must remain visible in the collapsed report summary.
    unknownChecks: results
      .flatMap((r) => r.checks)
      .filter((checkResult) => checkResult.status === 'unknown').length,
  };
