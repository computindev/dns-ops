...
Surfaces: web (TanStack Start + Hono on port 3000), collector route-boundary API (`apps/collector`), and public health HTTP (web `/api/health`, collector `/healthz` + `/readyz` on port 3001). SMTP STARTTLS is not exposed in the web UI; its safe local verification path drives the real collector route/repository with deterministic fixtures at the DNS, TCP, and TLS external boundaries.
...
Collector (needed for Domain 360 live collection, collector API verification, and the probe API, not for health.public / login / portfolio search UI chrome):
